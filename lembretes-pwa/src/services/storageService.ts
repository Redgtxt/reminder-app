...
// Interface para Web Storage (fallback)
...
// Implementação simples para React Native (sem AsyncStorage por enquanto)
...
class StorageService {
  ...
  async clear(): Promise<void> {
    try {
      ...
      // Remover chaves individualmente
  ...
  // Operações para dados do usuário (estatísticas, etc.)
  ...
  async getStorageSize(): Promise<number> {
    try {
      if (Platform.OS === 'web') {
        // Estimar tamanho no localStorage
        let totalSize = 0;
        for (const key in localStorage) {
          if (key.startsWith('@lembretes_pwa:')) {
            totalSize += localStorage[key].length;
          }
        }
        return totalSize;
      } else {
        // React Native - não há API direta, retornar 0
        return 0;
      }
    } catch (error) {
  ...
  async importData(jsonData: string): Promise<boolean> {
    try {
      ...
      if (importData.version && importData.data) {
        const { reminders, settings, userData } = importData.data;
        
        if (reminders) await this.saveReminders(reminders);
        if (settings) await this.saveSettings(settings);
        if (userData) await this.saveUserData(userData);
